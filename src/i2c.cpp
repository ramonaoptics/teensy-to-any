...
#include <i2c_t3.h>
#elif TEENSY_TO_ANY_HAS_I2C_T4
#include <i2c_driver_wire.h>
#elif TEENSY_TO_ANY_HAS_I2C_ARDUINO
#include <Wire.h>
#else
#error "No I2C implementation found"
#endif


#if TEENSY_TO_ANY_HAS_I2C_T3
int I2CMaster::init(int32_t baudrate, int32_t timeout_ms, int address_size,
...
int I2CMaster_T4::init(int32_t baudrate, int32_t timeout_ms, int address_size,
                    int address_msb_first) {
  if (address_size != 2 && address_size != 1)
    return ENOSYS;
  if (address_msb_first != 0)
...
int I2CMaster::begin_transaction(int slave_address) {
  ...
  if (slave_8bit_address)
    slave_address = slave_address >> 1;
  this->wire->beginTransmission(slave_address);
  return 0;
}
...
int I2CMaster::end_transaction(bool stop) {
  if (!this->is_initialized)
    return ENODEV;
  uint8_t error = this->wire->endTransmission(stop);
  return (int)error;
}
...
int I2CMaster::write_uint16(int slave_address, int register_address,
  ...
  this->wire->write((data >> 8) & 0xFF);
  this->wire->write((data >> 0) & 0xFF);
  uint8_t error = this->wire->endTransmission();
  return (int)error;
}
...
int I2CMaster::read_payload(int slave_address, int register_address,
  ...
  this->wire->beginTransmission(slave_address);
  this->_write_register_address(register_address);
  err = this->wire->endTransmission(false);
  if (err) {
    goto handle_error;
  }

  bytes_received = this->wire->requestFrom(slave_address, num_bytes);
...
int I2CMaster::read_uint16(int slave_address, int register_address,
  ...
  bytes_received = this->wire->requestFrom(slave_address, 2);
  if (bytes_received != 2) {
    err = 3;
    goto handle_error;
  }
...
int I2CMaster::read_uint8(int slave_address, int register_address,
  ...
  bytes_received = this->wire->requestFrom(slave_address, 1);
  if (bytes_received != 1) {
    err = 3;
    goto handle_error;
  }
